...
class TypeScriptAdapter(LanguageAdapter):
    ...
    def _generate_typescript_interface(self, schema: Dict) -> str:
        """Generate TypeScript interface from JSON schema."""
        interface_name = schema.get("title", "GeneratedInterface")
    ...
    def _get_typescript_type(self, schema: Dict) -> str:
        ...
        if schema_type == "string":
            ...
            items_type = self._get_typescript_type(schema.get("items", {"type": "string"}))
            return f"{items_type}[]"
        elif schema_type == "object":
            # Check if it has properties (nested object)
            if "properties" in schema:
                return self._generate_nested_interface(schema)
            return "Record<string, any>"
        else:
            return "any"

    def _generate_nested_interface(self, schema: Dict) -> str:
        """Generate nested interface for object properties."""
        properties = schema.get("properties", {})
        required = schema.get("required", [])
