...
class LanguageAdapterRegistry:
    ...
    def get_adapter(self, service_config: ServiceConfig, cache_key: Optional[str] = None) -> LanguageAdapter:
    ...
    async def detect_language(self, service_path: Path) -> Optional[str]:
        ...
        for language in self._auto_detection_order:
            ...
            try:
                # Create a temporary service config for detection
                from monorepo.config.models import ServiceConfig
    ...
    async def _can_handle_service(self, adapter: LanguageAdapter, service_path: Path) -> bool:
    ...
    async def validate_all_services(self, services: Dict[str, tuple[Path, ServiceConfig]]) -> Dict[str, Dict[str, Any]]:
    ...
    def _clear_cache_for_language(self, language: str) -> None:
        ...
        keys_to_remove = [k for k in self._adapter_cache.keys() if k.startswith(f"{language}:")]
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cached_languages": list(set(k.split(":")[0] for k in self._adapter_cache.keys())),
